...
/*function connectWebViewJavascriptBridge(a) {
    window.WebViewJavascriptBridge ? a(WebViewJavascriptBridge) : document.addEventListener("WebViewJavascriptBridgeReady",
    function() {
...
function pay_now(a) {
    ...
    b.amt = $("#amt").val(),
        b.openid = $("#openid").val(),
        b.sub_openid = $("#sub_openid").val(),
        b.userid = $("#userid").val(),
        b.opuid = $("#opuid").val(),
        b.discount = $("#discount").val(),
        $.ajax({
            ...
            success: function(a) {
                if ("0000" != a.respcd) alert(a.resperr),
                    isWx && safeCall(function() {
                ...
                else if ("weixin" == a.data.type)
                    if ("" != a.data.coupon_code) {
                        a.data.txamt = (a.data.txamt / 100).toFixed(2);
                        var b = template("back-template", a.data);
                        $("#wrap").append(b),
                            setTimeout(function() {
                                    $("#back").addClass("trans")
                                },
...
function getLocalTime(a) {
    return new Date(1e3 * parseInt(a)).toLocaleString().split(" ")[0].replace(/\//g, ".")
}!
function() {
    function a(a) {
...
var Zepto = function() {
    ...
    function m(a) {
        var b, c;
        return H[a] || (b = G.createElement(a), G.body.appendChild(b), c = getComputedStyle(b, "").getPropertyValue("display"), b.parentNode.removeChild(b), "none" == c && (c = "block"), H[a] = c),
            H[a]
    }

    function n(a) {
        return "children" in a ? F.call(a.children) : y.map(a.childNodes,
            function(a) {
    ...
    function v(a, b) {
        b(a);
        for (var c = 0,
                d = a.childNodes.length; d > c; c++) v(a.childNodes[c], b)
    }
    var w, x, y, z, A, B, C = [],
        D = C.concat,
        E = C.filter,
    ...
    return Y.matches = function(a, b) {
        ...
        Y.fragment = function(a, b, c) {
            ...
            return L.test(a) && (d = y(G.createElement(RegExp.$1))),
                d || (a.replace && (a = a.replace(M, "<$1></$2>")), b === w && (b = K.test(a) && RegExp.$1), b in T || (b = "*"), g = T[b], g.innerHTML = "" + a, d = y.each(F.call(g.childNodes),
                    function() {
                        g.removeChild(this)
                    })),
                f(c) && (e = y(d), y.each(c,
        ...
        Y.init = function(a, c) {
            ...
            if (!a) return Y.Z();
            if ("string" == typeof a)
                if (a = a.trim(), "<" == a[0] && K.test(a)) d = Y.fragment(a, RegExp.$1, c),
                    a = null;
                else {
        ...
        y.map = function(a, b) {
            var c, d, e, f = [];
            if (g(a))
                for (d = 0; d < a.length; d++) c = b(a[d], d),
                    null != c && f.push(c);
            else
                for (e in a) c = b(a[e], e),
                    null != c && f.push(c);
            return i(f)
        },
        y.each = function(a, b) {
        ...
        y.fn = {
            ...
            concat: function() {
                var a, b, c = [];
                for (a = 0; a < arguments.length; a++) b = arguments[a],
            ...
            ready: function(a) {
                return U.test(G.readyState) && G.body ? a(y) : G.addEventListener("DOMContentLoaded",
                        function() {
                            a(y)
                        }, !1),
                    this
...
window.Zepto = Zepto,
    ...
    function(a) {
        ...
        function o(b, c, d, e) {
            ...
            a.each(c,
                function(c, i) {
                    f = a.type(i),
                        e && (c = d ? e : e + "[" + (h || "object" == f || "array" == f ? c : "") + "]"), !e && g ? b.add(i.name, i.value) : "array" == f || !d && "object" == f ? o(b, i, d, c) : b.add(c, i)
                })
        }
        var p, q, r = 0,
            s = window.document,
    ...
    JDP.prototype = {
        ...
        getInfo: function(a) {
            ...
            this.fnCallback(b, a),
                this.android ? android.getInfo(b) : this.openIframe("native://getInfo?callback=" + b)
        },
        setTitle: function(a) {
            this.android ? android.setTitleName(a) : this.openIframe("native://setTitleName?titleName=" + a)
        },
        alert: function(a) {
            var b = ("globalCallback" + Math.random()).replace(/\./, "");
        ...
        confirm: function(a) {
            var b = ("globalCallback" + Math.random()).replace(/\./, "");
            this.fnCallback(b, a.callback),
                this.android ? android.confirm(a.text, a.sure, a.cancel, b) : this.openIframe("native://confirm?text=" + a.text + "&btn0Name=" + a.sure + "&btn1Name=" + a.cancel + "&callback=" + b)
        },
        close: function() {
        ...
        getCache: function(a) {
            var b = ("globalCallback" + Math.random()).replace(/\./, "");
            this.fnCallback(b, a.callback),
                this.android ? android.getCache(a.key, b) : this.openIframe("native://getCache? key=" + a.key + "&callback=" + b)
        },
        ...
        checkNetwork: function(a) {
            var b = ("globalCallback" + Math.random()).replace(/\./, "");
            this.fnCallback(b, a),
                this.android ? android.checkNetwork(b) : this.openIframe("native://checkNetwork?callback=" + b)
        },
        buriedPoint: function(a) {
            var b = a.once ? !0 : !1;
            b && this.android ? android.buriedPointOnce(a.eventId) : android.buriedPoint(a.eventId, a.eventLable),
                b && this.ios ? this.openIframe("native://buriedPoint?eventId=" + a.eventId) : this.openIframe("native://buriedPoint?eventId=" + a.eventId + "&eventLable=" + a.eventLable)
        },
        openAppModel: function(a) {
            var b = this.json2str(a.params) || "",
                c = a.name.toUpperCase(),
                d = ("globalCallback" + Math.random()).replace(/\./, "");
            return this.fnCallback(d, a.callback),
                "ACTIVITY" == a.name && this.android ? void android.start(a.params.url, d) : "ACTIVITY" == a.name && this.ios ? void this.openIframe("native://start?name=" + a.params.url + "&callback=" + d) : void(this.android ? android.start(c, JSON.stringify(a.params), d) : this.openIframe("native://start?name=" + c + b + "&callback=" + d))
        },
        shareText: function(a) {
            var b = ("globalCallback" + Math.random()).replace(/\./, ""),
                c = a.channel || "";
            this.fnCallback(b, a.callback),
                this.android ? android.shareText(a.text, c, b) : this.openIframe("native://shareText?text=" + a.text + "&jsonChannel=" + c + "&callback=" + b)
        },
        shareImage: function(a) {
            var b = ("globalCallback" + Math.random()).replace(/\./, "");
            a.channel || "";
            this.fnCallback(b, a.callback),
                this.android ? android.shareImage(a.startY, a.endY, a.channel, b) : this.openIframe("native://shareImage?startY=" + a.startY + "&endY=" + a.endY + "&jsonChannel=" + a.channel + "&callback=" + b)
        },
        shareUrl: function(a) {
            var b = ("globalCallback" + Math.random()).replace(/\./, ""),
                c = a.channel || "";
            this.fnCallback(b, a.callback),
                this.android ? android.shareWebPage(a.url, a.title, a.desc, c, b) : this.openIframe("native://shareWebPage?url=" + a.url + "&webTitle=" + a.title + "&webDesc=" + a.desc + "&jsonChannel=" + c + "&callback=" + b)
        },
        json2str: function(a) {
            var b = "";
        ...
        setMenu: function(a) {
            ...
            str = "[",
                ...
                str += "]",
                this.android ? android.setTitleMenu(str) : WebViewJavascriptBridge.callHandler("setMenu", b)
        },
        pay: function(a) {
            var b = ("globalCallback" + Math.random()).replace(/\./, ""),
                c = JSON.stringify(a.params);
            this.fnCallback(b, a.callback),
                delete a.callback,
                this.android ? android.pay(c, b) : this.openIframe("native://commonPay?params=" + c + "&callback=" + b)
        }
    },
    function(a) {
        var b = function(a, b, c) {
            var d = this,
                e = $("#" + b);
            c["int"] = c["int"] ? c["int"] : 6,
                c.decimal = c.decimal ? c.decimal : 2;
        ...
        var i = $(".container").index();
            ...
            f.on("touchstart", "i",
                ...
                f.on("touchend touchcancel touchmove", "i",
                    function(a) {
                        a.preventDefault(),
                ...
                f.on("tap", "i",
                    function() {
                        ...
                        $(".real-pay span").text((Number($("#amount").text()||0).toFixed(2)-index.yxyhmoney)<0?0.00:(Number($("#amount").text()||0).toFixed(2)-index.yxyhmoney));
                    })
        };
        b.prototype.insert = function(a, b, c) {
                if (t = $(".active").find(".amount-number span").text(), "0" != t || "." == a) {
                    ...
                    if (index.orderid=="0") {
                    	$(".active").find(".amount-number span").text($(".active").find(".amount-number span").text()+a);
					}
                    
                    if ($(".active").parent().attr("id")=="shurujine") {
                    	$("#pay").addClass("active");
					}
                }
            },
            b.prototype.del = function(a) {
                	t = $(".active").find(".amount-number span").text();
                	
                	if (index.orderid=="0") {
                		t && $(".active").find(".amount-number span").text(t.substring(0, t.length - 1));
					}
                	
                	if ($(".active").parent().attr("id")=="shurujine") {
                		"" == $(".active").find(".amount-number span").text() && $("#pay").removeClass("active")
					}
            },
            b.prototype.hideBoard = function(a) {
                	...
                	if (index.cardtype=="1") {
						...
						index.cardIfPass();index.getRealPayNum();
					}
            },
            b.prototype.submitData = function(a) {
                "" != a.text() && a.text() > 0 && $("#pay").hasClass("active") && ($("#amt").val((100 * a.text()).toFixed(0)),index.cardIfPass(),index.cardtype=="1"?index.getCardUse():index.getRealPayNum(),index.order())
            },
            b.prototype.showCoupon = function(a) {
                if ($(".coupon").length > 0) {
                    var b = $("#coupon-num").text() / 10;
                    $("#coupon").text((a.text() * (1 - b)).toFixed(2)),
                        $("#coupon-pay").text((a.text() * b).toFixed(2))
                }
            },
            $(".amount").on("tap",
                function() {            	
